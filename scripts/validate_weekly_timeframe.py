"""
MKUMARAN Trading OS — Weekly Timeframe Backtest

Tests Wyckoff and Confluence-2 on weekly bars (5-day) on the PSU-excluded
Nifty 100 universe. Compares to daily-bar baselines from 2026-05-08.

Hypothesis: weekly bars reduce noise, produce higher-quality Wyckoff/Confluence
signals than daily bars, leading to better PF and profitable-ticker rate.

Criteria doc: docs/strategy_validation/weekly_timeframe_criteria.md
Baselines:    Wyckoff daily PF 0.70 | Confluence-2 daily PF 0.68
...
Usage:
    ...
    python scripts/validate_weekly_timeframe.py --workers 4 --resume
"""
...
logger = logging.getLogger("weekly_timeframe")

# ── PSU exclusion list (locked — same as all prior tests) ─────────────────
...
ENGINES = [
    {
        ...
        "strategy":    "wyckoff",
        "min_engines": 2,
        "baseline_pf": 0.70,   # daily PSU-excluded (2026-05-08)
        "baseline_label": "daily PSU-excl 0.70",
    },
    {
        "name":        "confluence2_weekly",
        "strategy":    "confluence",
        "min_engines": 2,
        "baseline_pf": 0.68,   # daily PSU-excluded (2026-05-08)
        "baseline_label": "daily PSU-excl 0.68",
    },
]

DAYS            = 1095
CAPITAL         = 100_000
SLIPPAGE_WEEKLY = 0.005   # 0.5% per side for weekly bars (wider than daily 0.3%)
DEFAULT_WORKERS = 4
...
def run_ticker(
    ...
    try:
        ...
        result = run_backtest(
            ...
            days=days,
            capital=CAPITAL,
            slippage_pct=SLIPPAGE_WEEKLY,
            interval="1wk",              # weekly bars — key difference
...
def aggregate_engine(results: list[dict], engine: dict) -> dict:
    ...
    # Tier thresholds per weekly_timeframe_criteria.md
    if pf_med is None:
        tier = "INCONCLUSIVE"
    elif pf_med >= 1.2 and profitable_pct >= 40 and total_trades >= 100:
        tier = "TIER_1"
    ...
    return {
        ...
        "baseline_label":  engine["baseline_label"],
        "pf_lift":         pf_lift,
        "tier":            tier,
    }
...
def write_report(
    ...
    lines = [
        ...
        "## Engine Summary",
        "",
        "| Engine | Trades | Median PF | Baseline (daily) | Lift | WR | Profitable | Tier |",
        "|---|---|---|---|---|---|---|---|",
    ]
    for s in engine_summaries:
        ...
        pf_str   = str(s["pf_median"]) if s["pf_median"] else "—"
        lines.append(
            "| {} | {:,} | {} | {} | {} | {} | {}/{} ({:.0f}%) | {} |".format(
                s["engine"],
                s["total_trades"],
                pf_str,
                "{} ({})".format(s["baseline_pf"], s["baseline_label"]),
                lift_str,
                wr_str,
    ...
    inc = [s for s in engine_summaries if s["tier"] == "INCONCLUSIVE"]

    if t1:
        lines.append("**TIER 1** — Weekly timeframe rescues engine (walk-forward next):")
        for s in t1:
            lines.append("  - {}: PF {}, {}% profitable".format(
                s["engine"], s["pf_median"], s["profitable_pct"]))
    if t2:
        lines.append("**TIER 2** — Weekly improves but insufficient (MWA filter next):")
        for s in t2:
            lines.append("  - {}: PF {}, {}% profitable".format(
                s["engine"], s["pf_median"], s["profitable_pct"]))
    if t3:
        lines.append("**TIER 3** — Weekly timeframe does not rescue:")
        for s in t3:
            lines.append("  - {}: PF {} (daily baseline {}), lift {}".format(
                s["engine"], s["pf_median"] or "—", s["baseline_pf"],
                "{:+.3f}".format(s["pf_lift"]) if s["pf_lift"] is not None else "—",
            ))
    if inc:
        lines.append("**INCONCLUSIVE** — insufficient signal mass:")
        for s in inc:
            lines.append("  - {}: {} trades (min 50 required)".format(
                s["engine"], s["total_trades"]))
...
def main() -> None:
    ...
    parser.add_argument("--engines", default="",
                        help="Comma-separated engine names (wyckoff_weekly, confluence2_weekly)")
    ...
    if args.engines:
        ...
        name_map = {"wyckoff": "wyckoff_weekly", "confluence2": "confluence2_weekly",
                    "confluence_2": "confluence2_weekly"}
        normalised = {name_map.get(r, r) for r in requested}
        engines = [e for e in ENGINES if e["name"] in normalised]
        if not engines:
            logger.error("No matching engines for: %s (valid: %s)",
                         args.engines, ", ".join(e["name"] for e in ENGINES))
            return
    else:
        engines = ENGINES

    reports_dir = Path("reports") / "weekly_timeframe"
    reports_dir.mkdir(parents=True, exist_ok=True)

    logger.info(
        "Weekly timeframe: %d tickers, %d engines, workers=%d, slippage=%.1f%%",
        len(universe), len(engines), args.workers, SLIPPAGE_WEEKLY * 100,
    )
    ...
    print("\n" + "=" * 72)
    print("WEEKLY TIMEFRAME — RESULTS (vs daily PSU-excluded baseline)")
    print("=" * 72)
    print("{:<22} {:>7} {:>10} {:>10} {:>7} {:>8}".format(
        "Engine", "Trades", "Median PF", "Daily base", "Lift", "Tier"
    ))
    print("-" * 72)
    for s in engine_summaries:
        lift_str = "{:+.3f}".format(s["pf_lift"]) if s["pf_lift"] is not None else "  —  "
