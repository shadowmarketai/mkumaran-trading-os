...
function RankChange({ current, prev }: { current: number; prev: number | null }) {
  ...
  if (diff > 0) {
    return (
      <span className="flex items-center gap-0.5 text-trading-bull text-xs font-mono">
  ...
  if (diff < 0) {
    return (
      <span className="flex items-center gap-0.5 text-trading-bear text-xs font-mono">
    ...
    >
      ...
      {rankings.length > 0 && (
        <div className="grid grid-cols-4 gap-3">
          ...
          <GlassCard className="!p-3 text-center">
            ...
            <p className="text-xl font-mono font-bold text-white">{holdings.length}</p>
          </GlassCard>
          <GlassCard className="!p-3 text-center" glowColor="bull">
            <p className="text-[10px] text-slate-500 uppercase tracking-wider">Buy Signals</p>
            <p className="text-xl font-mono font-bold text-trading-bull">{buySignals.length}</p>
          </GlassCard>
          <GlassCard className="!p-3 text-center" glowColor="bear">
            <p className="text-[10px] text-slate-500 uppercase tracking-wider">Sell Signals</p>
            <p className="text-xl font-mono font-bold text-trading-bear">{sellSignals.length}</p>
          </GlassCard>
        </div>
      )}
      ...
      {rankings.length > 0 ? (
        ...
        <GlassCard className="text-center py-12">
          <Rocket size={32} className="mx-auto mb-3 text-slate-600" />
      ...
      {holdings.length > 0 && (
        <GlassCard className="!p-4">
          <p className="text-[10px] text-slate-500 uppercase tracking-wider mb-2">Current Portfolio</p>
          <div className="flex flex-wrap gap-2">
            {holdings.map((ticker) => (
              <span
                key={ticker}
                className="px-2.5 py-1 rounded-md text-xs font-medium text-white bg-trading-card border border-trading-border"
              >
                {ticker}
              </span>
            ))}
