...
export default function ProgressBar({ current, min, max, label, isShort, pnlPct }: ProgressBarProps) {
  ...
  return (
    <div className="w-full">
      {label && <p className="text-[9px] text-slate-400 mb-1 font-mono">{label}</p>}
      <div className="relative">
        ...
        <div className="h-1.5 bg-slate-100 rounded-full overflow-visible relative border border-slate-200">
          <motion.div
            initial={{ width: 0 }}
            animate={{ width: `${pct}%` }}
          ...
          >
            <div className={cn(
              'w-2.5 h-2.5 rounded-full border-[1.5px] border-white',
              barColor,
              isProfit ? 'shadow-bull' : 'shadow-bear'
            )} />
          </motion.div>
        </div>
        <div className="flex justify-center mt-1">
          <span className="text-[9px] font-mono text-slate-700 font-semibold tabular-nums">{current.toFixed(1)}</span>
        </div>
      </div>
    </div>
  );
